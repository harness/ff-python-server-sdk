...
class FeatureConfig:
    ...
    Attributes:
        ...
        environment (str):
        feature (str):
        state (FeatureState): The state of a flag either off or on
        kind (FeatureConfigKind):
        variations (List['Variation']):
        default_serve (Serve): Describe the distribution rule and the variation that should be served to the target
        off_variation (str):
        rules (Union[Unset, List['ServingRule']]):
        prerequisites (Union[Unset, List['Prerequisite']]):
        variation_to_target_map (Union[Unset, List['VariationMap']]):
